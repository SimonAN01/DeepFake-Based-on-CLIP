...
# 用于添加当前目录到系统路径，确保可以导入项目中的其他模块，.表示当前目录
sys.path.append('.')
...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str): # eg. dataset_name: FaceForensics++
        ...
        if not os.path.exists(self.config['dataset_json_folder']):
            # # 调试信息：执行到路径修正处
            print("\033[1;33m" + "="*50 + "\033[0m")  # 黄色粗体分隔线
            print("\033[1;32m" + "[DEBUG POINT] 正在执行数据集路径修正" + "\033[0m")  # 绿色粗体
            print(f"dataset_json_folder: \033[1;34m{self.config['dataset_json_folder']}\033[0m")  # 蓝色粗体路径
            ...
            print("\033[1;33m" + "="*50 + "\033[0m")  # 黄色粗体分隔线
            print("\033[1;32m" + "[DEBUG POINT] 修正后数据集路径" + "\033[0m")  # 绿色粗体
            print(f"dataset_json_folder: \033[1;34m{self.config['dataset_json_folder']}\033[0m")  # 蓝色粗体路径
            print("\033[1;33m" + "="*50 + "\033[0m")
        try:
