...
class SVDResidualLinear(nn.Module):
    ...
    def forward(self, x):
        if hasattr(self, 'U_residual') and hasattr(self, 'V_residual') and self.S_residual is not None:
            # Reconstruct the residual weights
            residual_weight = self.U_residual @ torch.diag(self.S_residual) @ self.V_residual
            # Total weights is the fixed main weights plus the residual
            weight = self.weight_main + residual_weight
        else:
            # If residual components are not set, use only the main weights
...
def replace_with_svd_residual(module, r):
    if isinstance(module, nn.Linear):
        ...
        # Perform SVD on the original weights
        U, S, Vh = torch.linalg.svd(module.weight.data, full_matrices=False)

        # Determine r based on the rank of the weights matrix
        r = min(r, len(S))  # Ensure r does not exceed the number of singular values

        # Keep top r singular components (main weights)
        U_r = U[:, :r]      # Shape: (out_features, r)
        S_r = S[:r]         # Shape: (r,)
        Vh_r = Vh[:r, :]    # Shape: (r, in_features)

        # Reconstruct the main weights (fixed)
        weight_main = U_r @ torch.diag(S_r) @ Vh_r

        # Set the main weights
