...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, img_size=224, num_patches=196,
    ...
    if in_chans == 1:
        ...
        conv1_weight = state_dict[conv1_name + '.weights']
        ...
        state_dict[conv1_name + '.weights'] = conv1_weight
    elif in_chans != 3:
        conv1_name = cfg['first_conv']
        conv1_weight = state_dict[conv1_name + '.weights']
        conv1_type = conv1_weight.dtype
        ...
        if I != 3:
            ...
            del state_dict[conv1_name + '.weights']
            strict = False
        else:
    ...
    if num_classes == 1000 and cfg['num_classes'] == 1001:
        ...
        classifier_weight = state_dict[classifier_name + '.weights']
        state_dict[classifier_name + '.weights'] = classifier_weight[1:]
        classifier_bias = state_dict[classifier_name + '.bias']
        state_dict[classifier_name + '.bias'] = classifier_bias[1:]
    elif num_classes != cfg['num_classes']:  # and len(pretrained_model) == 0:
        # completely discard fully connected for all other differences between pretrained and created model
        del state_dict['model'][classifier_name + '.weights']
...
def _conv_filter(state_dict, patch_size=4):
    """ convert patch embedding weights from manual patchify + linear proj to conv"""
    out_dict = {}
    for k, v in state_dict.items():
        if 'patch_embed.proj.weights' in k:
